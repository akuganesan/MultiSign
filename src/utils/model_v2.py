...
class Decoder(nn.Module):
    def __init__(self, hidden_size, pose_size, trajectory_size,
               ...
               use_lang=False, use_attn=False, num_layers=1, epoch=100, device="cuda"):
    ...
    def forward(self, h, time_steps, gt, bert_tokens=None, epoch=np.inf):
        if self.start_zero:
        ...
        # reset with the new unit and hidden state
        for t in range(time_steps): 
            mask = self.tf(epoch, h.shape[0]).view(-1, 1).to(x.device)
            x = mask * gt[:, t, :] + (1-mask) * x
            
            if self.use_attn:
                h_, t_att_w, s_att_w = self.attention(self.dropout(h.unsqueeze(1)), H, bert_tokens)
                h_ = self.linear(h_) 
    ...
    def sample(self, h, time_steps, start, bert_tokens=None):
        #x = torch.rand(h.shape[0], self.input_size).to(h.device).to(h.dtype)
        ## starting point for the decoding 
    ...
    def attention(self, h, H, bert_tokens):
        for i in range(self.num_layers):
            temporal_h, temporal_att_weights = self.temporal_attention[i](h, H, H)
            h = temporal_h + h # + temporal_h
            spacial_h, spacial_att_weights = self.spacial_attention[i](h, bert_tokens, bert_tokens)
            h = spacial_h + h # + temporal_h
            h = self.attention_mlps[i](h)
        return h, temporal_att_weights, spacial_att_weights   

        
        
class DecoderCurriculum(nn.Module):
    def __init__(self, hidden_size, pose_size, trajectory_size,
               use_h=False, start_zero=False, use_tp=True,
               use_lang=False, use_attn=False, num_layers=1, epoch=100, device="cuda"):
        super(DecoderCurriculum, self).__init__()
        self.input_size = pose_size + trajectory_size
        self.cell = DecoderCell(hidden_size, pose_size, trajectory_size,
                                use_h=use_h, use_tp=use_tp, use_lang=use_lang)
    ...
    def forward(self, h, time_steps, gt, bert_tokens=None, epoch=np.inf):
        ...
        # do 3 iterations in the future and then reset with 
        # the obtained hidden state. This might actually migrate
        #  the hands down problem in each sequence
        for t in range(3):
            x, h = self.cell(x, h)
    ...
    def sample(self, h, time_steps, start, bert_tokens=None):
        ...
        # do 2 * 3 iterations in validation and test 
        # for robustness 
        for t in range(6):
            x, h = self.cell(x, h)
...
#         if self.use_attn:
#             if self.use_lang:
#                 X, t, s = self.attention(self.dropout(X), XH)
#             else:
#                 X, t, s = self.attention(self.dropout(X), H)
#             return self.linear(X), t, s
...
#         return x, temporal_att_weights, spacial_att_weights
